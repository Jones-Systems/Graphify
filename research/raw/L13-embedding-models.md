...
|BAAI/bge-m3|tool|https://huggingface.co/BAAI/bge-m3|MIT|High (rel. Jan 2024, FlagEmbedding)|2|1|2|2|3|H|MTEB English R 48.82 dense (LOWEST of set) despite 568M params; 8192 ctx, dense+sparse+ColBERT, 100+ langs; FlagEmbedding forces FP32 on CPU (calls model.float()); ~2.27GB fp32 weights push encode peak near/over the 3GiB MemAvailable floor; value ONLY if cross-lingual or built-in hybrid lexical needed. GPU not required - but CPU-impractical at our scale.|
...
## CPU throughput at our scale (~200k chunks, ~60-80M tokens)

Anchors (measured): nomic-embed-text 137M via llama.cpp/Ollama = 850 tok/s short inputs, 620 tok/s @2k-token inputs on a 4-core Intel N100 mini PC (toolbrain.net build log); ONNX single-query warm latency bge-small-en-v1.5 6.1ms vs snowflake-arctic-embed-s 4.1ms (pdf-mcp benchmark docs). Our 16-core EPYC Genoa is roughly an order of magnitude above an N100; ONNX Runtime multi-threaded batched encode scales accordingly.
...
## GPU flag
NONE of the compared models requires a GPU. All run CPU-only. BGE-M3 is the only candidate whose default library path is CPU-hostile (forced fp32, 2.27GB resident, slowest throughput); it remains legal-but-impractical here. No candidate needs owner approval: licenses Apache-2.0 or MIT, all self-hostable offline after one-time HF download.
