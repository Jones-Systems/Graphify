...
Recorded 2026-08-25. The report compares CPU-only, offline-capable
`llama-server` profiles. Public benchmark and model data are retained as
recorded; no later currentness or deployment fit is claimed.
...
- Thread, batch, slot, and context values must be selected from a
  deployment-bound benchmark; `--metrics`, an authentication boundary, and a
  constrained bind address remain relevant public configuration surfaces.
...
- These public measurements cannot be scaled reliably to a different core
  count, memory-channel allocation, or shared environment. Gate any profile on
  deployment-target `llama-bench` and `scripts/server-bench.py` results.
...
|Qwen3-8B quant ladder|model profile|https://huggingface.co/Qwen/Qwen3-8B-GGUF|Apache-2.0|stable on recorded date|5|4|4|4|1|H|Recorded package sizes plus the worked KV formula produce an illustrative ~8.1 GiB total at 32k context; throughput is unmeasured.|
|Qwen3-14B quant ladder|model profile|https://huggingface.co/Qwen/Qwen3-14B-GGUF|Apache-2.0|stable on recorded date|4|3|4|5|1|M|Recorded package sizes plus the worked KV formula produce an illustrative ~12 GiB total at 32k context; the public 14B benchmark does not establish throughput on other hardware.|
...
## Verdict
The recorded comparison identified `llama-server` with a Qwen3-8B Q4_K_M
profile as a candidate and Qwen3-14B Q4_K_M as a larger alternative. It did
not establish a safe thread count, slot count, context size, memory policy, or
throughput. Any use requires fresh version verification and deployment-target
memory, latency, concurrency, and quality measurements.
