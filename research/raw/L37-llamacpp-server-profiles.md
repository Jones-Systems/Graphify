...
# Lane L37 — T6 local-llm: llama.cpp `llama-server` on CPU-only EPYC Genoa (16-core VPS)

Researched 2026-08-25. Environment: Debian 13, 16C EPYC Genoa, **no GPU**, 64 GB RAM, hard floor **MemAvailable ≥ 3072 MiB during any burst**, Python 3.13, offline/self-hostable required. All sources primary (llama.cpp master docs/releases, AMD dev blog, HF API file sizes, measured community llama-bench logs).
...
- Recommended for our box: `-t 16 -tb 16 -np 4 -c 16384` (4096/slot) or `-np 2`; `-b 2048 -ub 512` defaults fine; `--metrics` for Prometheus; `--api-key` + loopback bind.

## 2. Memory formula vs our 3072 MiB floor
Peak RSS ≈ **W + K + C + O**:
...
| Deployment | W (GiB) | KV q8_0 (GiB) | Total est. (GiB) | Headroom vs floor |
|---|---|---|---|---|
| Qwen3-4B Q4_K_M | 2.33 | 2.39 (144 KiB/tok f16 ×0.53) | ~5.7 | trivially safe |
| Qwen3-8B Q4_K_M | 4.68 | 2.39 | ~8.1 | safe |
| Qwen3-14B Q4_K_M | 8.38 | 2.66 (160 KiB/tok) | ~12.0 | safe |
| Qwen3-14B Q8_0 | 14.62 | 2.66 | ~18.3 | safe |
| Llama-3.1-8B Q4_K_M | 4.58 | 2.13 (128 KiB/tok) | ~7.7 | safe |
| Phi-4 Q4_K_M | 8.43 | ~3.32 (200 KiB/tok, 40L×1280) | ~12.8 | safe |

Capacity verdict: even 14B Q8_0 at 64k ctx (~24 GiB) leaves MemAvailable ≈ 37+ GiB ≫ 3,072 MiB floor. **Memory capacity is never the binding constraint; DDR bandwidth is.** Residency: `--load-mode mmap+mlock` pins weights (counts fully against MemAvailable — still fine at ≤24 GiB); bare mmap risks page-evict latency spikes under burst pressure from tantivy/LanceDB jobs.
...
|`--load-mode mmap+mlock` residency|strategy|https://github.com/ggml-org/llama.cpp/blob/master/tools/server/README.md|MIT|stable|4|2|1|0|0|H|--mlock/--mmap deprecated in favor of --load-mode (auto/mmap/mlock/mmap+mlock); mlock counts against our MemAvailable floor — fine ≤24 GiB|
...
|Qwen3-8B quant ladder ⭐|model profile|https://huggingface.co/Qwen/Qwen3-8B-GGUF|Apache-2.0|stable|5|4|4|4|1|H|Sizes verified; ~8.1 GiB total @32k ctx q8-KV; est. 15–35 t/s on our box [I]; best latency/quality point for short pipeline outputs|
|Qwen3-14B quant ladder|model profile|https://huggingface.co/Qwen/Qwen3-14B-GGUF|Apache-2.0|stable|4|3|4|5|1|M|Sizes verified; ~12 GiB @32k q8-KV; 14.66B-class measured 29.5 t/s Q4 on full 9554 → est. 8–22 t/s on ours [I]; quality ceiling pick|
...
## Verdict
Top pick: **llama-server (pin v0.3.0/b10621) + Qwen3-8B-Instruct Q4_K_M**, `-t 16 -np 4 -c 16384 -fa on -ctk q8_0 -ctv q8_0 --load-mode mmap+mlock --numa distribute --metrics` — ~8 GiB total footprint (floor-safe by 50+ GiB), Apache-2.0, OAI-compatible loopback API consumable from Python 3.13 with zero keys/offline; upgrade path to Qwen3-14B Q4_K_M (~12 GiB) for async/batch quality, and rebuild once with `-DGGML_ZENDNN=ON` for ~2× prompt processing on our Genoa silicon.
Integration sketch: build native (`cmake -DGGML_NATIVE=ON`), run as systemd unit on 127.0.0.1:8080 behind existing query-rewrite/entity-extraction stage; gate rollout on on-host `llama-bench` (tg128 @ -t 16/32) + `scripts/server-bench.py -np 4` confirming MemAvailable stays ≥ 3072 MiB and tg ≥ 12 t/s on 8B Q4_K_M.
