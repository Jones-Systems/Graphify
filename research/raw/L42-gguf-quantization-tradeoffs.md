...
Investigated: 2026-08-25. Question: Q4_K_M vs Q5_K_M vs Q8_0 vs f16 (Q6_K as reference) for entity/relation extraction — published perplexity/degradation, JSON-schema adherence, and RAM math. Target assumptions: CPU-only local execution, a declared memory floor, offline operation, and permissive tooling. Claims were not independently revalidated during remediation.
...
Set the per-run cap from current available memory minus the declared floor; the prior approximate cap is illustrative, not portable. Larger profiles may require exclusive scheduling. Disk demand follows GGUF weight size, while runtime memory additionally includes cache and buffers.
...
|imatrix quants (WT-calibrated; IQ4_XS)|technique|https://github.com/ggml-org/llama.cpp/pull/4861|MIT|mature (since 2024-06)|4|4|3|3|2|M-H|L3-8B Q4_K_M+imatrix ΔPPL 0.151 vs 0.175, KLD .0282 vs .0313; IQ4_XS 4.14 GiB KLD .0363 ≈ plain Q4_K_S; needs calibration corpus (a representative target corpus can supply it)|
